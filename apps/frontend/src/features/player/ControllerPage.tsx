...
export function ControllerPage(): React.ReactNode {
  ...
  if (!data) return <div className="p-6 text-sm text-zinc-500">Cargando...</div>;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      <header className="border-b border-zinc-200 px-4 py-3">
        <h1 className="text-sm font-semibold">Controlador — Sala {code}</h1>
        <p className={`text-xs ${room.connected ? "text-emerald-600" : "text-red-600"}`}>{room.connected ? "Conectado" : "Desconectado"}</p>
      </header>

      <main className="flex flex-1 flex-col gap-4 p-4">
        <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-center">
          <p className="text-xs uppercase tracking-widest text-zinc-500">Diapositiva</p>
          <p className="text-3xl font-bold">
            {room.currentSlide + 1} / {data.slides.length}
          </p>
          <p className="mt-1 text-xs text-zinc-500">{slide ? `${slide.elements.length} elementos` : "Sin diapositiva"}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-xl border border-zinc-200 bg-white py-4 text-sm font-medium"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-xl bg-zinc-900 py-4 text-sm font-medium text-white"
          >
        ...
        {slide && (
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-widest text-zinc-500">Resaltar objeto</h2>
              ...
              {slide.elements
                ...
                .map((el) => (
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 rounded-xl border py-3 text-xs ${room.highlightedId === el.id ? "border-amber-400 bg-amber-50 text-amber-900" : "border-zinc-200 bg-white"}`}
                  >
                    <FiTarget /> {el.type} {el.id.slice(0, 4)}
                  </button>
                ))}
              {slide.elements.filter((e) => e.highlightable).length === 0 && <p className="text-xs text-zinc-500">Sin objetos resaltables.</p>}
            </div>
            ...
            <button
              ...
              className="mt-3 w-full rounded-xl border border-zinc-200 bg-white py-3 text-sm font-medium disabled:opacity-40"
            >
              Activar animación
            </button>
          </div>
