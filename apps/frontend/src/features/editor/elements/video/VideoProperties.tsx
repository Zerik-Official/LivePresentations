  ...
  return (
    <div className="space-y-3">
      <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300">
        URL video
        <input value={props.src ?? ""} onChange={(e) => onPatch({ propsPatch: { src: e.target.value } })} placeholder="https://..." className="mt-1 w-full rounded-lg border bg-(--input-bg) border-(--input-border) text-(--input-text) px-3 py-2 text-sm outline-none focus:border-zinc-900 dark:focus:border-zinc-400" />
      </label>
      ...
      <div className="grid grid-cols-3 gap-2">
        <label className="flex items-center gap-1.5 text-xs font-medium text-zinc-700 dark:text-zinc-300">
          <input type="checkbox" checked={Boolean(props.autoplay)} onChange={(e) => onPatch({ propsPatch: { autoplay: e.target.checked } })} className="accent-zinc-900 dark:accent-white" /> Autoplay
        </label>
        <label className="flex items-center gap-1.5 text-xs font-medium text-zinc-700 dark:text-zinc-300">
          <input type="checkbox" checked={Boolean(props.loop)} onChange={(e) => onPatch({ propsPatch: { loop: e.target.checked } })} className="accent-zinc-900 dark:accent-white" /> Loop
        </label>
        <label className="flex items-center gap-1.5 text-xs font-medium text-zinc-700 dark:text-zinc-300">
          <input type="checkbox" checked={props.muted ?? true} onChange={(e) => onPatch({ propsPatch: { muted: e.target.checked } })} className="accent-zinc-900 dark:accent-white" /> Muted
        </label>
      </div>
    </div>
  );
